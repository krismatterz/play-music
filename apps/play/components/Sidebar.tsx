...
const Sidebar: React.FC<SidebarProps> = ({ className }) => {
  ...
  return (
    <div className={`hidden md:flex flex-col h-full bg-black p-6 ${className}`}>
      <div className="mb-8">
        ...
        {routes.map((route) => (
          <Link
            ...
            className={`flex items-center gap-4 px-4 py-3 rounded-lg transition ${
              route.active ? 'bg-white/10 text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'
            }`}
          >
            {route.icon}
